...
class ResultViewModel extends ViewModelBase {
  ...
  void onReady(UserEntry _user) {
    ...
    var _currentRange = bmiRange.where((element) => element.startValue <= _currentEntry!.bmi && element.endValue >= _currentEntry!.bmi).first;
    currentCategory = _currentRange.category;
    _logger.d("ResultViewModel", "Got Category to BMI-Result: ${_currentEntry?.bmi}, Category: ${currentCategory}");
  }
